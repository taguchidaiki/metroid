...
bool HelloWorld::init()
{
	...
	lpEffectCtl.AddEffection("Laser01.efk", 13.0f);
	lpEffectCtl.AddEffection("Homing_Laser01.efk");
	/**
...
void HelloWorld::update(float delta)
{
	...
	if (count % 300 == 0)
	{
		/**
			...
			您通过指定比例读取效果文件。
		*/
		auto effect = lpEffectCtl.GetEffectData("Laser01.efk");
		if (effect != nullptr)
		{
			/**
	...
	if (count % 300 == 120)
	{
		...
		您读取效果文件。
		*/
		auto effect = lpEffectCtl.GetEffectData("Homing_Laser01.efk");
		if (effect != nullptr)
		{
			/**
			エミッターを生成し、パラメーターを設定してレイヤーに追加します。
